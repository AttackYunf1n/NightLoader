...
local Settings = {}
local SettingsFile = "Nightfall_Settings.json"
...
function Nightfall:Loader(onSuccess)
    if CoreGui:FindFirstChild("NightfallLoader") then CoreGui.NightfallLoader:Destroy() end

    local ScreenGui = Create("ScreenGui", { Name = "NightfallLoader", Parent = CoreGui, ZIndexBehavior = Enum.ZIndexBehavior.Sibling })
    ...
    Create("TextLabel", { Parent = Content, BackgroundTransparency = 1, Position = UDim2.new(0, 0, 0.1, 0), Size = UDim2.new(1, 0, 0, 30), Font = Enum.Font.GothamBold, Text = "PHANTOM <font color='rgb(220,30,30)'>NIGHTFALL</font>", TextColor3 = Theme.Text, TextSize = 24, RichText = true })
    ...
    local PassBox, PassFrame = CreateInput("Password", "Lock", true) PassFrame.Position = UDim2.new(0, 20, 0.50, 0)

    UserBox.Text = LocalPlayer.Name

    if isfile and isfile("Nightfall_Key.txt") and readfile then
        pcall(function()
            PassBox.Text = readfile("Nightfall_Key.txt")
        end)
    end
    ...
    GetKeyBtn.MouseButton1Click:Connect(function()
        if setclipboard then
            setclipboard("https://docs.luarmor.net/")
            Nightfall:Notify("Key System", "Link copied to clipboard!", 2)
        end
    end)
    ...
    LoginBtn.MouseButton1Click:Connect(function()
        if PassBox.Text == "" or UserBox.Text == "" then
            ...
            if writefile then
                pcall(function()
                    writefile("Nightfall_Key.txt", PassBox.Text)
                end)
            end
...
function Nightfall:Notify(title, text, duration)
    local ScreenGui = CoreGui:FindFirstChild("NightfallUI")
...
function Nightfall:Window(title)
    if CoreGui:FindFirstChild("NightfallUI") then
        CoreGui.NightfallUI:Destroy()
    end
    ...
    function WindowObj:Tab(name, icon)
        ...
        TabPage.Left.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() UpdateCanvas(TabPage.Left) end)
        TabPage.Right.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() UpdateCanvas(TabPage.Right) end)
        ...
        function TabObj:Section(title, side)
            local Parent = (side == "Right") and TabPage.Right or TabPage.Left
            local SectionObj = {}
